...
def load_actionability_thresholds(path: Path | None = None) -> ActionabilityThresholds:
    ...
    return ActionabilityThresholds(
        delta_hours_accounting_month=float(minimums.get("delta_hours_accounting_month", 0.25)),
        eff_pct_accounting_month=float(minimums.get("eff_pct_accounting_month", 2.0)),
        eff_idx_accounting_month=float(minimums.get("eff_idx_accounting_month", 0.2)),
        confidence=float(minimums.get("confidence", 0.15)),
        require_all=bool(scoring.get("require_all", True)),
        fallback_status=str(scoring.get("fallback_status") or "na"),
        fallback_reason_code=str(scoring.get("fallback_reason_code") or "BELOW_ACTIONABILITY_THRESHOLD"),
    )
