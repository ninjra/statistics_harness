...
def _als_cp(tensor: np.ndarray, rank: int, max_iter: int = 50) -> tuple[list[np.ndarray], float]:
    dims = tensor.shape
    factors = [np.random.default_rng(42).standard_normal((d, rank)) for d in dims]
