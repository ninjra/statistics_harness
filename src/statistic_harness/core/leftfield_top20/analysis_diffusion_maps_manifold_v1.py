...
def run(ctx) -> PluginResult:
    ...
    artifacts = [artifact(ctx, PLUGIN_ID, "diffusion_maps_embedding.json", {"eigenvalues": eigvals[:10].tolist(), "eigengap": eigengap, "embedding_sample": emb[:200].tolist()})]
    ...
    return PluginResult("ok", "Computed diffusion maps manifold embedding", {"rows_used": int(x.shape[0]), "eigengap": eigengap}, findings, artifacts, None)
