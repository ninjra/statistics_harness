...
from .common import HAS_SKLEARN, KFold, LogisticRegression, RandomForestClassifier, artifact, build_config, degraded, finding, prepare_data, roc_auc_score
...
def run(ctx) -> PluginResult:
    ...
    kf = KFold(n_splits=folds, shuffle=True, random_state=int(getattr(ctx, "run_seed", 0) or 0))
    probs = np.zeros((x_in.shape[0], 2), dtype=float)
    valid_mask = np.zeros(x_in.shape[0], dtype=bool)
    for train, test in kf.split(x_in):
