...
class Plugin:
    def run(self, ctx) -> PluginResult:
        ...
        sequence_col = _pick_column(
            ...
            role_by_name,
            {"master_id"},
            ["master", "sequence", "chain", "workflow", "batch", "group", "case"],
            lower_names,
        )
        ...
        if not start_col or (not queue_col and not eligible_col):
            return PluginResult(
                "ok",
                "Tail isolation not applicable",
                {"tail_rows": 0, "tail_hours_total": 0.0},
                [
                    {
                        "kind": "tail_isolation",
        ...
        if work.empty:
            return PluginResult("ok", "No valid timestamps", {"tail_rows": 0}, [], [], None)
        ...
        tail = work["__eligible_wait_sec"] > threshold
        tail_work = work.loc[tail].copy()

        if tail_work.empty:
            return PluginResult(
                "ok",
                "No tail rows above threshold",
                {"tail_rows": 0, "tail_hours_total": 0.0},
                [
                    {
                        "kind": "tail_isolation",
                        "measurement_type": "not_applicable",
                        "reason": "No rows exceed the tail threshold.",
                        "threshold_seconds": threshold,
                        "columns": [
                            col
                            for col in [process_col, module_col, user_col, sequence_col]
                            if col
                        ],
                    }
                ],
        ...
        def _emit_dimension(name: str, col: str | None) -> None:
            if not col or col not in tail_work.columns:
                return
            temp = tail_work.copy()
            ...
            grouped = (
                temp.groupby("__key")
                .agg(
                    tail_rows=("__key", "size"),
                    tail_wait_sec=("__eligible_wait_sec", "sum"),
                )
                .reset_index()
            )
            grouped = grouped.sort_values(
                ["tail_wait_sec", "__key"], ascending=[False, True]
            )
        ...
        _emit_dimension("sequence", sequence_col)

        tail_hours = float(tail_work["__eligible_wait_sec"].sum() / 3600.0)

        artifacts_dir = ctx.artifacts_dir("analysis_tail_isolation")
        out_path = artifacts_dir / "tail_isolation.json"
        write_json(
            out_path,
            {
                "summary": {
                ...
                "tail_rows": int(tail_work.shape[0]),
                "tail_hours_total": tail_hours,
            },
        )
        artifacts = [
            PluginArtifact(
                ...
                type="json",
                description="Tail isolation summary",
            )
        ]

        return PluginResult(
            "ok",
            "Isolated tail-eligible wait segments",
            {
                "tail_rows": int(tail_work.shape[0]),
                "tail_hours_total": tail_hours,
            },
