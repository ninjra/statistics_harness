...
class Plugin:
    def run(self, ctx) -> PluginResult:
        ...
        rf = RandomForestRegressor(n_estimators=int(ctx.settings.get("n_estimators", 50)), random_state=ctx.run_seed)
        rf.fit(X, y)
        importances = rf.feature_importances_
        knockoff = X.apply(lambda col: rng.permutation(col.to_numpy()))
        rf_knock = RandomForestRegressor(n_estimators=int(ctx.settings.get("n_estimators", 50)), random_state=ctx.run_seed)
        ...
        return PluginResult("ok", "Computed RF knockoff selection", {"selected": selected_count}, findings, artifacts, None)
